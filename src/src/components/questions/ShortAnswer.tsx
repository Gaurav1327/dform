...
export type ShortAnswerEditableProps = {
    question: QuestionType,
    onChange: (formFieldId: string, question: QuestionType) => void
}

export type ShortAnswerComponentProps = {
    question: QuestionType,
    isPreview?: boolean
}

export function ShortAnswerEditable({ question, onChange }: ShortAnswerEditableProps) {
        ...
        <div className="flex flex-col items-center rounded-lg p-10 my-4 m-auto border-[1px] border-black w-full bg-white">
            ...
            <div id="form-metadata" className="flex flex-col w-full items-center gap-3">
                ...
                <div className='flex items-center justify-end w-full px-4 pt-10 gap-1'>
                    <ToggleSwitch id="shortAnswer" checked={question.required} onChange={() => handleChange("required", !question.required)}/>
...
export function ShortAnswerComponent({ question }: ShortAnswerComponentProps) {
